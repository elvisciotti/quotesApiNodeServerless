...
console.log("*** RECREATE SCRIPT INITIALIZED ***");
...
db.serialize(() => {
  ...
  fs.readdirSync(__dirname + "/csv").map((csvFile) => {
    ...
    const insertStatement = db.prepare(
      "INSERT INTO quotes (quote, author, tag) VALUES (?, ?, ?)"
    );
    fs.readFileSync(`${__dirname}/csv/${csvFile}`, "utf8")
      ...
      .forEach((csvLine) => {
        for (let regexprIndex in CSV_ATTEMPTS_MATCH) {
          ...
          if (
            match &&
            !ALREADY_ADDED.includes(match[1]) &&
            match[1].length > 10
          ) {
            insertStatement.run([match[1], match[2], csvFile.slice(0, -4)]);
